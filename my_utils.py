...
def error_handling(func_name, by, df_, feature_name, fig, ax):
    ...
    if feature_name not in df_.columns:
        if feature_name:
            print_error_msg(
                f'{func_name}(): There is no such feature in dataset.')
            fig.delaxes(ax)
        else:
            print_error_msg(
                f'{func_name}(): Please, specify the feature_name.')
            fig.delaxes(ax)
    ...
    if by not in ['month', 'year', 'month and year']:
        print_error_msg(
            'plot_observations_by(): Only by `year` or `month` or `month and year` are acceptable.')
        fig.delaxes(ax)
        return

    if feature_name and by != 'month and year' and df_[feature_name].dtype != 'object':
        print_error_msg(
            f'{func_name}(): For `year` or `month` modes only categorical features are acceptable.')
        fig.delaxes(ax)
        return -1

    if feature_name and by == 'month and year' and df_[feature_name].dtype == 'object':
        print_error_msg(
            f'{func_name}(): For `month and year` mode only numeric features are acceptable.')
        fig.delaxes(ax)
        return -1
...
def plot_observations_by(by=None, df_=None, feature_name=None, fig=None, ax=None):
    ...
    if feature_name == 'month':
        ...
        order = pd.Categorical(
            df_[feature_name].value_counts().index, ordered=True)
    ...
    if feature_name:
        ...
        if by in ['year', 'month']:
            ...
            if by == 'year':
                observations = pd.crosstab(df_[feature_name], df_[
                                           'year']).reindex(labels=order)
            else:
                lbls_order = order
...
def plot_yes_proportion_by(by=None, df_=None, feature_name=None, fig=None, ax=None):
    ...
    if feature_name:
        ...
        if by == 'year':
            ...
            yes_proportion_by_year = df_.groupby([feature_name, 'year'])[
                'y'].mean().unstack()
            ...
            yes_proportion_by_year = yes_proportion_by_year.reindex(
                labels=order)
            ...
            yes_proportion_by_month = df_.groupby([feature_name, 'month'])[
                'y'].mean().unstack()
        ...
        # ONLY FOR NUMERIC FEATURES
            ...
            yes_proportion_by_month_and_year = df_.groupby(['month', 'year'])[
                'y'].mean().unstack()

            order = months_order
            yes_proportion_by_month_and_year = yes_proportion_by_month_and_year.reindex(
                labels=order)
...
def plot_distribution(df_=None, feature_name=None, feature_type=None, num_gr=10, plot_options={}):
    ...
    if feature_type == 'categorical':
        ...
        axes[0].set_title(
            f'`{feature_name}` distribution', fontsize=16, pad=20)

        sns.histplot(df_[feature_name], stat='density', lw=0, ax=axes[0])
        sns.kdeplot(df_[feature_name], color='#ffb500', lw=3, ax=axes[0])

        # Plot feature stats
        _, mean_val, _, min_val, q25, q50, q75, max_val = np.round(
            df_[feature_name].describe(), 2)
        ...
        axes[0].text(x_coor, y_coor, feature_stats_text,
        ...
        print_error_msg(
            'plot_distribution(): Only `categorical` and `numeric` are acceptable.')
        fig.delaxes(ax)
        return
...
def plot_yes_proportion(df_=None, feature_name=None, feature_type=None, num_gr=10, plot_options={}):
    ...
    if feature_type == 'categorical':
        ...
        df_yes_proba = (
            df_yes_proba[1] / (df_yes_proba[0] + df_yes_proba[1])).reindex(custom_order)
        ...
        df_bins_yes = pd.DataFrame(
            pd.concat([df_[feature_name], bins_series, df_['y']], axis=1))

        yes_answers = df_bins_yes.groupby(bins_series.name)[
            'y'].agg(lambda y: y.eq(1).sum())
        ...
        print_error_msg(
            'plot_yes_proportion(): Only `categorical` and `numeric` are acceptable.')
        fig.delaxes(ax)
        return
